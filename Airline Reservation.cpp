...
int main() {
	...
	cout << logo;
	//prints out choices for type of reservation
	...
	gotoxy(86, 12);
	cin >> type;
	while (type[0] != '2' && type[0] != '1' || type[1] > 0) { //Input Validation
		gotoxy(1, 12);
	...
	//checks what type of reservation was two-way and asks and then ask user for return flight schedule
	switch (type2)
	{
	case '2':
		system("cls");
		...
		cout << "\t\t      " << char(204);  for (int i = 0; i < 74; i++) { cout << char(205); }  cout << char(185) << endl;
		cout << "\t\t      " << char(186) << setw(75) << char(186) << endl;
		cout << "\t\t      " << char(186) << setw(35); Color(240); cout << "Year: "; Color(241); cout << setw(40) << char(186) << endl;
		cout << "\t\t      " << char(186) << setw(75) << char(186) << endl;
		cout << "\t\t      " << char(186) << setw(35); Color(240); cout << "Month: "; Color(241); cout << setw(40) << char(186) << endl;
		cout << "\t\t      " << char(186) << setw(75) << char(186) << endl;
		cout << "\t\t      " << char(186) << setw(35); Color(240); cout << "Day: "; Color(241); cout << setw(40) << char(186) << endl;
		cout << "\t\t      " << char(186) << setw(75) << char(186) << endl;
		cout << "\t\t      " << char(200);      for (int i = 0; i < 74; i++) { cout << char(205); }      cout << char(188) << endl;
		gotoxy(58, 15);
		getline(cin >> ws, date); //input of return date year
		do {
			...
			{
				stringstream str_stream_object(date);
				str_stream_object >> year1;
				if (year1 < year || year1>year + 1) {
					gotoxy(1, 15);
					cout << "\t\t      "; Color(240); cout << char(186) << setw(35) << "Invalid Year: "; Color(241); cout << setw(40) << char(186) << endl;
					gotoxy(58, 15);
					getline(cin >> ws, date);
					is_num_bool = 0;
				}
		...
		cout << "        Year: ";
		gotoxy(58, 17);
		getline(cin >> ws, date); //input return date month
		do {
			...
			if (date[0] == '-') // validate if negative
			{
				is_num = 0;
			}
			...
			{
				stringstream str_stream_object(date);
				str_stream_object >> month1; //convertion of string numerics into int
				if (year1 == year) { //if return shcedule is in the same year, return month can't be any previous months of the initial departure date
					if (month1 < month || month1 > 12) {
						gotoxy(1, 17);
						cout << "\t\t      " << char(186) << setw(35); Color(240); cout << "Invalid Month: "; Color(241); cout << setw(40) << char(186) << endl;
						gotoxy(58, 17);
						getline(cin >> ws, date);
						is_num_bool = 0;
					}
		...
		do {
			...
			{
				stringstream str_stream_object(date);
				str_stream_object >> day1; //convertion of string numerics into int
				//get the month input and validate the input day and limit it by the corresponding month
				switch (month1) {
				case 1: case 3: case 5: case 8: case 10: case 12:
					while (day1 > 31) {
						gotoxy(1, 19);
						cout << "\t\t      " << char(186) << setw(35); Color(240); cout << "Invalid Day: "; Color(241); cout << setw(40) << char(186) << endl;
						gotoxy(58, 19);
						getline(cin >> ws, date);
						is_num_bool = 0;
					}
		...
		case 'a': case 'A':
			schedule = "12:00am";
			hr2 = 0; // 12am is 0 in 24 hr format
			break;

		case 'b': case 'B':
			schedule = "2:00am";
			hr2 = 2; // 2am is 2 in 24 hr format
			break;

		case 'c': case 'C':
			schedule = "4:00am";
			hr2 = 4; // 4am is 4 in 24 hr format
			break;

		case 'd': case 'D':
			schedule = "6:00am";
			hr2 = 6; // 6am is 6 in 24 hr format
			break;

		case 'e': case 'E':
			schedule = "8:00am";
			hr2 = 8; // 8am is 8 in 24 hr format
			break;

		case 'f': case 'F':
			schedule = "10:00am";
			hr2 = 10; // 10am is 10 in 24 hr format
			break;

		case 'g': case 'G':
			schedule = "12:00pm";
			hr2 = 12; // 12pm is 12 in 24 hr format
			break;

		case 'h': case 'H':
			schedule = "2:00pm";
			hr2 = 14; // 2pm is 14 in 24 hr format
			break;

		case 'i': case 'I':
			schedule = "4:00pm";
			hr2 = 16; // 4pm is 16 in 24 hr format
			break;

		case 'j': case 'J':
			schedule = "6:00pm";
			hr2 = 18; // 6pm is 18 in 24 hr format
			break;

		case 'k': case 'K':
			schedule = "8:00pm";
			hr2 = 20; // 8pm is 20 in 24 hr format
			break;

		case 'l': case 'L':
			schedule = "10:00pm";
			hr2 = 22; // 10pm is 22 in 12 hr format
			break;
		}
		while (hr2 <= hr) {
			gotoxy(1, 41);
	...
	//x+=4;//change
	//gotoxy(60, 18);
	//Color(240);cout  << "P " << change;Color(241);cout <<endl;
